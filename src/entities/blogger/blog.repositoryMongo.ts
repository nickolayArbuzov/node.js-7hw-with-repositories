...
import { BlogModel } from './schema/blog.interface';
...
@Injectable()
export class BlogRepositoryMongo implements IBlogRepositoryInterface {
  constructor(
    @Inject('BLOG_MONGOOSE')
    private blogRepository: Model<BlogModel>,
  ) {}
  ...
  async findAllPostsByBlogId(id: string, query: QueryBlogDto) {
    ...
    if(blog) {
      ...
      const all = await repo
        ...
        .take(pageSize)
        .orderBy(`posts.${query.sortBy ? query.sortBy : queryDefault.sortBy}`, sortDirection) 
        .getOne()
  ...
  async createBlogger(dto: CreateBloggerDto) {
    ...
    await this.bloggerRepository.insert(newBlogger);
    return newBlogger*/
  }
  ...
  async updateBlogger(id: string, dto: UpdateBloggerDto) {
    ...
    if(donorBlogger) {
      ...
      await this.bloggerRepository.update(id, newBlogger);
      return newBlogger;
    } else {
      throw new HttpException('Blogger not found', HttpStatus.NOT_FOUND);
    }*/
  ...
  async deleteBlogger(id: string) {
    /*const donorBlogger = await this.blogRepository.findOne({where: {id: id}});
    if(donorBlogger) {
